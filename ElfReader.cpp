...
ElfReader::ElfReader()
    : source_(nullptr), name_(nullptr), phdr_num_(0), phdr_mmap_(NULL),
      phdr_table_(NULL), phdr_size_(0), load_start_(NULL), load_size_(0),
      load_bias_(0), loaded_phdr_(NULL) {}
...
bool ElfReader::ReadProgramHeader() {
  ...
  phdr_table_ =
      reinterpret_cast<Elf_Phdr *>(reinterpret_cast<char *>(mmap_result));
...
bool ElfReader::ReserveAddressSpace(uint32_t padding_size) {
  ...
  // (load_bias_ + phdr.vaddr), so load_bias_ = load_start -
  // phdr.vaddr(min_addr)
 ...
 */
static int _phdr_table_set_load_prot(const Elf_Phdr *phdr_table, int phdr_count,
                                     uint8_t *load_bias, int extra_prot_flags) {
 ...
 */
static int _phdr_table_set_gnu_relro_prot(const Elf_Phdr *phdr_table,
                                          int phdr_count, uint8_t *load_bias,
                                          int prot_flags) {
  const Elf_Phdr *phdr = phdr_table;
  const Elf_Phdr *phdr_limit = phdr + phdr_count;
...
void ElfReader::ApplyPhdrTable() {
  const Elf_Phdr *phdr_limit = phdr_table_ + phdr_num_;
  memcpy((void *)loaded_phdr_, (void *)phdr_table_,
         (uintptr_t)phdr_limit - (uintptr_t)phdr_table_);
  return;
}
